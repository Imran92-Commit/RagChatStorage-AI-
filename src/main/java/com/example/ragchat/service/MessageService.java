...
public class MessageService implements MessageReadService, MessageWriteService {
    ...
    private final ChatSessionRepository sessRepo;

    public MessageService(ChatMessageRepository m, ChatSessionRepository s) {
        this.msgRepo = m;
        this.sessRepo = s;
    }

    @Transactional
    public ChatMessage addMessage(UUID sessionId, AddMessageRequest req) {
        ChatSession session = sessRepo.findById(sessionId).orElseThrow(() -> new NotFoundException("Session not found"));
        ChatMessage m = new ChatMessage();
        m.setSession(session);
        m.setSender(ChatMessage.Sender.valueOf(req.getSender().toUpperCase()));
