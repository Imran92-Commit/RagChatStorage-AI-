...
public class SessionService implements SessionReadService, SessionWriteService {
    ...
    private final MessagePurger purger;
    private final AuditLogger audit;

    public SessionService(ChatSessionRepository repo, MessagePurger purger, AuditLogger audit) {
        this.repo = repo;
        this.purger = purger;
        this.audit = audit;
    }
    ...
    public ChatSession createSession(CreateSessionRequest req) {
        ChatSession s = ChatSession.create(req.getUserId(), req.getTitle());
        ChatSession saved = repo.save(s);
        audit.sessionCreated(saved.getId(), saved.getUserId());
        return saved;
    }

    public ChatSession get(UUID id) {
        return repo.findById(id).orElseThrow(() -> new NotFoundException("Session not found"));
    }
    ...
    public void delete(UUID id) {
        ChatSession s = get(id);
        purger.purgeMessages(s);
        repo.delete(s);
        audit.sessionDeleted(id, s.getUserId());
    }
}
